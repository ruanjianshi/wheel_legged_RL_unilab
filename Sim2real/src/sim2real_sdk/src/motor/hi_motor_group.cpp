...
{
    HiMotorGroup::HiMotorGroup(std::shared_ptr<livelybot_serial::robot> rbPtr, std::vector<int> mapIndex, std::vector<std::string> names, std::vector<double> lowest, std::vector<double> highest)
        : RobotMotorGroup(rbPtr, mapIndex, names)
    {
        std::vector<std::shared_ptr<HtdwMotor>> allMotors, lowerBodyMotors, upperBodyMotors;
        ...
        for (auto i = 0; i < mapIndex.size(); i++)
        {
            ...
            std::cout << "HiMotorGroup create [" << i << "/" << mapIndex.size() << "] index: " << mapIndex[i] << " name: " << names[i] << " motor (" << lowest[mapIndex[i]] << "~" << highest[mapIndex[i]] << ")" << std::endl;
            if (mapIndex[i] < 12)
            {
