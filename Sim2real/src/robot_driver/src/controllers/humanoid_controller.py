...
class HumanoidDriver:
    ...
    def __init__(self):
        ...
        # 自旋自动前进参数
        self.auto_forward_enable = rospy.get_param(
            "~auto_forward/enable", True
        )  # 是否启用自旋自动前进
        self.auto_forward_speed = rospy.get_param("~auto_forward/speed", 0.18)  # 自动前进速度
        self.auto_forward_rotation_threshold = rospy.get_param(
            "~auto_forward/rotation_threshold", 0.05
        )  # 旋转阈值
        ...
        # 订阅遥控手柄话题
        self.joy_subscriber = rospy.Subscriber("/joy_input", Joy, self.joy_callback, queue_size=1)

        # 订阅IMU话题
        self.imu_subscriber = rospy.Subscriber(self.imu_topic, Imu, self.imu_callback, queue_size=1)

        # 初始化本地发布者
        self.local_publisher = None
        if self.local_enable:
            self.local_publisher = rospy.Publisher(self.local_topic, Joy, queue_size=1)

        # IMU补偿向量发布者
        self.imu_compensation_publisher = rospy.Publisher(
            "/imu_compensation", Vector3, queue_size=1
        )
    ...
    def imu_compensation(self):
        ...
        # PD控制器补偿计算
        # X方向补偿使用pitch角度
        self.v_imu_compensation[0] = (
            pitch * self.pd_x_kp + (pitch - self.pitch_last_error) * self.pd_x_kd
        )

        # Y方向补偿使用roll角度
        self.v_imu_compensation[1] = (
            roll * self.pd_y_kp + (roll - self.roll_last_error) * self.pd_y_kd
        )
