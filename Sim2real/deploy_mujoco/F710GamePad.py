...
class F710GamePad:
    ...
    def GetValues(self):
        ...
        if self.os.lower() == "windows":
            axis0, axis1, axis2, axis3, axis4, axis5 = 0, 1, 2, 3, 4, 5
        else:
            axis0, axis1, axis2, axis3, axis4, axis5 = 0, 1, 3, 4, 2, 5

        # sleep(1/self.freq)
        eventlist = pygame.event.get()

        for event in eventlist:
            if event.type == pygame.locals.JOYAXISMOTION and (
                event.axis == axis0 or event.axis == axis1
            ):
                self.x0, self.y0 = (
                    round(self.joystick0.get_axis(axis0) * self.joyL_max, self.round_output),
                    -1 * round(self.joystick0.get_axis(axis1) * self.joyL_max, self.round_output),
                )

            if event.type == pygame.locals.JOYAXISMOTION and (
                event.axis == axis2 or event.axis == axis3
            ):
                self.x1, self.y1 = (
                    round(self.joystick0.get_axis(axis2) * self.joyR_max, self.round_output),
                    -1 * round(self.joystick0.get_axis(axis3) * self.joyR_max, self.round_output),
                )

            if event.type == pygame.locals.JOYAXISMOTION and (event.axis == axis4):
                self.triggerL = round(
                    (self.joystick0.get_axis(axis4) + 1) * self.trigL_max / 2, self.round_output
                )

            if event.type == pygame.locals.JOYAXISMOTION and (event.axis == axis5):
                self.triggerR = round(
                    (self.joystick0.get_axis(axis5) + 1) * self.trigR_max / 2, self.round_output
                )
    ...
    def GetInput(
        ...
        if joyL:
            output.append(list(inputs[0:2]))
        if joyR:
            output.append(list(inputs[2:4]))
        if trigL:
            output.append(inputs[4])
        if trigR:
            output.append(inputs[5])
        if hat:
            output.append(list(inputs[6]))
        if buttons:
            output.append(inputs[7])
        else:
            pass
        return output
