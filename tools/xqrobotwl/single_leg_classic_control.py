...
用法:
  uv run mjpython tools/xqrobotwl/single_leg_classic_control.py                # 单次运行
  uv run mjpython tools/xqrobotwl/single_leg_classic_control.py --search       # 网格搜索增益
  uv run mjpython tools/xqrobotwl/single_leg_classic_control.py --render video/single_leg/classic.mp4
"""
...
def run_once(
    ...
    for i in range(n_steps):
        ...
        wheel_acc = sign * (kp_p * (theta - theta_ref) + kd_p * theta_dot) + kc * (vx_cmd - vx)
        wheel_vel = np.clip(wheel_vel + (wheel_acc / WHEEL_R) * dt, -wheel_max, wheel_max)
        # ── roll: 自由腿配重位置 PD ──
        l_roll = L_ROLL_BASE - kp_r * roll_err - kd_r * roll_dot

        ctrl = np.array([l_roll, L_PITCH, L_KNEE, 0.0, R_ROLL, R_PITCH, R_KNEE, wheel_vel])
        data.ctrl[:] = ctrl
...
def main() -> None:
    ...
    ap.add_argument(
        "--vx", type=float, nargs="+", default=[0.0, 0.3, -0.3, 0.0], help="分段 vx 命令"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=60.0,
        help="支撑腿位置执行器刚度 (实验: kp=60 柔性最佳, 高kp刚性反倒)",
    )
    ap.add_argument(
        "--leg_kd", type=float, default=0.0, help="支撑腿位置执行器阻尼 (实验: kd>0 有害, 保持0)"
    )
    ap.add_argument(
        "--sign", type=float, default=-1.0, help="pitch 控制方向 (±1, 实验标定: 后仰→轮子向前)"
    )
    ap.add_argument(
        "--lean_deg", type=float, default=30.0, help="机身侧压角 deg (30°不稳, 10-25°更稳)"
    )
        ...
        for kp_p, kd_p, kc in itertools.product([40, 60, 90], [8, 12, 18], [2, 4, 8]):
            for kp_r in [1.0, 2.0, 4.0]:
                r = run_once(
                    ...
                    6.0,
                    args.sim_dt,
                    wheel_kv=args.wheel_kv,
                    wheel_max=args.wheel_max,
                    leg_kp=args.leg_kp,
                    leg_kd=args.leg_kd,
                    sign=args.sign,
                ...
                print(
                    f"  {tag:52s} → hold {r['hold_time']:5.2f}s ({r['fall_cause']}) |CoM_dy|={r['mean_com_dy']:.4f}"
                )
    ...
    print("=" * 72)
    print(
        f"  参数: kp_p={args.kp_p} kd_p={args.kd_p} kp_r={args.kp_r} kd_r={args.kd_r} kc={args.kc} k_theta={args.k_theta}"
    )
    ...
    if n > 0:
        ...
        print(
            f"  z 范围: {tr['z'][:n].min():.2f}~{tr['z'][:n].max():.2f}, "
        ...
        for i, vc in enumerate(args.vx):
            ...
            if s1 > s0:
                print(
                    f"    vx_cmd={vc:+.2f}: 实际 {tr['vx'][s0:s1].mean():+.2f} (漂移 {tr['vx'][s0:s1].mean() - vc:+.2f})"
                )
    if r["hold_time"] >= 2.0:
        print("✅ 通过: 单轮稳定 ≥2s")
    else:
