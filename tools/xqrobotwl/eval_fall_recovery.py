...
用法:
  ...
  uv run mjpython tools/xqrobotwl/eval_fall_recovery.py --run <run_dir> --pose 1
"""
...
def _set_fixed_pose_provider(env, pose: int) -> None:
    ...
    class _FixedPoseProvider(fr.XqRobotWLFallRecoveryDRProvider):
        ...
        def build_reset_plan(self, env: object, env_ids: np.ndarray) -> fr.ResetPlan:
            ...
            base_z = np.full(num_reset, fr._LYING_Z, dtype=np.float64) + rng.uniform(
                -0.02, 0.02, size=num_reset
            )
            quats = np.zeros((num_reset, 4), dtype=np.float64)
            for i in range(num_reset):
                q = fr._pose_quat(self._pose)
                dq = fr._quat_from_euler(
                    rng.uniform(-0.3, 0.3), rng.uniform(-0.3, 0.3), rng.uniform(-0.3, 0.3)
                )
                quats[i] = fr._quat_mul(q, dq)
            legs = np.clip(
                np.array([0.1, 0.15, 0.15, -0.1, -0.15, -0.15])
                * rng.uniform(0.5, 1.5, size=(num_reset, 6)),
                -0.85,
                0.85,
            )
...
def main() -> None:
    ...
    pose_tag = (
        f"  (姿态 {args.pose}: {POSES[args.pose]})"
        if args.pose in (0, 1, 2, 3)
        else "  (随机 4 姿态)"
    )
    print("=" * 60)
