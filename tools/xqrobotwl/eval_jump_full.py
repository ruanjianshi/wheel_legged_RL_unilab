...
def _first_sustained(mask, start=0, min_steps=3):
    """Return the first index of a sustained boolean run.

    A short wheel-height threshold crossing is common during load transfer and
    must not be labelled as takeoff or touchdown.
    """
    run = 0
    for i in range(start, len(mask)):
        run = run + 1 if mask[i] else 0
        if run >= min_steps:
            return i - min_steps + 1
    return None
...
def _phase_stats(rows):
    ...
    gyro = np.mean(
        [
            (float(r["gyro_x"]) ** 2 + float(r["gyro_y"]) ** 2 + float(r["gyro_z"]) ** 2) ** 0.5
            for r in rows
        ]
    )
...
def main() -> int:
    ...
    p.add_argument(
        "--config",
        default="",
        help="optional current task YAML; overrides the checkpoint's saved environment config",
    )
    ...
    ensure_registries()
    ov = (
        config_env_overrides(args.config) if args.config else trained_env_overrides(args.checkpoint)
    )
    ...
    try:
        ...
        with torch.no_grad():
            for step in range(total):
                ...
                up_z = float(
                    np.asarray(env._backend.get_sensor_data("upvector")).reshape(-1, 3)[0, 2]
                )
                lz = float(
                    np.asarray(env._backend.get_sensor_data("left_wheel_world_pos")).reshape(-1, 3)[
                        0, 2
                    ]
                )
                rz = float(
                    np.asarray(env._backend.get_sensor_data("right_wheel_world_pos")).reshape(
                        -1, 3
                    )[0, 2]
                )
                contact_l = 1.0 if lz < args.wheel_radius + 0.02 else 0.0
                contact_r = 1.0 if rz < args.wheel_radius + 0.02 else 0.0
                ...
                rows.append(
                    {
                        ...
                        "trigger": trig,
                        "base_z": float(bp[2]),
                        "L_hip_roll": float(dof[0]),
                        "L_hip_pitch": float(dof[1]),
                        "L_knee": float(dof[2]),
                        "R_hip_roll": float(dof[3]),
                        "R_hip_pitch": float(dof[4]),
                        "R_knee": float(dof[5]),
                        "L_wheel_rads": float(dv[6]),
                        "R_wheel_rads": float(dv[7]),
                        "base_roll": roll,
                        "base_pitch": pitch,
                        "base_yaw": yaw,
                        "linvel_x": float(lv[0]),
                        ...
                        "gyro_z": float(gyro[2]),
                        "up_z": up_z,
                        "wheel_contact_L": contact_l,
                        "wheel_contact_R": contact_r,
                        "jump_phase": phase,
        ...
        both_contact = [min(r["wheel_contact_L"], r["wheel_contact_R"]) for r in rows]
        both_air = [max(r["wheel_contact_L"], r["wheel_contact_R"]) < 0.5 for r in rows]
        ...
        # ── 1. 站立姿态 (触发前 settle) ─────────────────────────────
        stand_sel = [
            r for r in rows[: args.settle] if min(r["wheel_contact_L"], r["wheel_contact_R"]) > 0.5
        ]
        if stand_sel:
            s = _phase_stats(stand_sel)
            j = s["j"]
            issues = classify_pose(
                s["z"],
                s["up"],
                j,
                [s["roll"], s["pitch"], 0],
            ...
            print(
                f"    base_z={s['z']:.3f} (目标 {args.standing}, {'✅' if height_ok else '❌'}) "
        ...
        crouch = min(
            (r for r in on_sel if min(r["wheel_contact_L"], r["wheel_contact_R"]) > 0.5),
            key=lambda r: r["base_z"],
            default=None,
        )
        if crouch:
            j = np.array(
            ...
            print(
                f"    base_z={z_c:.3f} (下蹲深度 {args.standing - z_c:+.3f}, "
                f"膝屈 {knee_bend:.2f} rad, {'✅ 深蹲' if z_c < 0.45 else '⚠️ 下蹲不足' if z_c < 0.52 else '❌ 未下蹲'})"
            )
            print(
                f"    关节={_fmt_joint(j)} 异常: {classify_pose(z_c, crouch['up_z'], j, [crouch['base_roll'], crouch['base_pitch'], 0], 0, True, args.standing) or '✅ 无'}"
            )
        else:
            print("\n[2] 触发→下蹲: ❌ 未检测到下蹲 (可能未起跳)")

        # ── 3. 起跳 (轮离地瞬间) ────────────────────────────────────
        takeoff_mask = [trigs[i] > 0.5 and both_air[i] for i in range(n)]
        air_start = _first_sustained(takeoff_mask, min_steps=3)
        # 起跳前最后接地步
        lift = None
        if air_start is not None:
            for i in range(air_start, -1, -1):
                if both_contact[i] > 0.5:
                    lift = i
                    break
        if air_start is not None:
            ...
            if lift is not None:
                ...
                print(
                    f"    起跳前接地 @step {lift}: z={rows[lift]['base_z']:.3f} 膝={j[2]:+.2f},{j[5]:+.2f} "
        ...
        air_sel = [
            r
            for r in rows
            if max(r["wheel_contact_L"], r["wheel_contact_R"]) < 0.5 and r["trigger"] > 0.5
        ]
        if air_sel:
            ...
            j = a["j"]
            issues = classify_pose(
                a["z"], a["up"], j, [a["roll"], a["pitch"], 0], a["gyro"], False, args.standing
            )
            ...
            print(
                f"    up_z={a['up']:.3f} base_pitch={a['pitch']:+.2f} 轮速={a['dw']:.1f} rad/s "
        ...
        land_i = None
        if air_start is not None:
            landing_mask = [c > 0.5 for c in both_contact]
            land_i = _first_sustained(landing_mask, start=air_start + 3, min_steps=3)
        if land_i is not None:
            ...
            avg_w = np.mean(
                [
                    abs(float(r["L_wheel_rads"])) + abs(float(r["R_wheel_rads"]))
                    for r in rows[pre : land_i + 1]
                ]
            )
            print(f"\n[5] 落地 @step {land_i}")
            print(
                f"    落地冲击 vx/vz={rows[land_i]['linvel_x']:.2f}/{rows[land_i]['linvel_z']:.2f} m/s "
                f"({'✅' if impact < 2.0 else '⚠️ 冲击偏大'})"
            )
            print(
                f"    落地缓冲: 落地后最低 z={absorb_z:.3f} "
                f"({'✅ 缓冲正常' if absorb_z > 0.25 else '❌ 塌陷'})"
            )
            print(
                f"    落地时轮速 |w|={avg_w:.1f} rad/s "
        ...
        if land_i is not None:
            ...
            for i in range(land_i, min(land_i + 100, n)):
                if (
                    abs(rows[i]["base_z"] - args.standing) < 0.12
                    and rows[i]["up_z"] > 0.85
                    and both_contact[i] > 0.5
                ):
                    rec_i = i
                    break
            if rec_i is not None:
                ...
                j = r["j"]
                issues = classify_pose(
                    r["z"], r["up"], j, [r["roll"], r["pitch"], 0], r["gyro"], True, args.standing
                )
                # 微动平衡: 恢复后 20 步水平漂移
                x0 = float(rows[rec_i]["linvel_x"])
                y0 = float(rows[rec_i]["linvel_y"])
                print(f"\n[6] 恢复站立 @step {rec_i} (落地后 {rec_i - land_i} 步)")
                print(
                    f"    base_z={r['z']:.3f} up_z={r['up']:.3f} |gyro|={r['gyro']:.2f} "
                    f"(稳定 {'✅' if r['gyro'] < 1.0 else '❌ 晃动'})"
                )
                print(f"    关节={_fmt_joint(j)} 异常: {issues if issues else '✅ 无'}")
            else:
                print(
                    "\n[6] 恢复站立: ❌ 落地后 {:.0f} 步内未恢复到稳定站立 (z≈{:.2f}, up>0.85, 轮着地)".format(
                        min(100, n - land_i), args.standing
                    )
                )
        else:
            print("\n[6] 恢复站立: ❌ 未落地无法评估")
        ...
        checks = [
            (
                "站立姿态",
                "✅"
                if locals().get("stand_sel") and height_ok_s
                else "⚠️"
                if locals().get("stand_sel")
                else "❌",
            ),
