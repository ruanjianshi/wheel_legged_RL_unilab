...
from verify_jump import config_env_overrides, load_actor, trained_env_overrides  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--settle", type=int, default=50, help="trigger-off settle steps before 1st jump"
    )
    ...
    ap.add_argument(
        "--hold", type=int, default=10, help="consecutive steps that define stable standing"
    )
    ...
    try:
        ...
        with torch.no_grad():
            for ep in range(args.episodes):
                ...
                for j in range(args.jumps):
                    ...
                    drift = 0.0
                    streak = 0
                    # Recovery must persist at the end of the complete window.
                    # Declaring success at the first stable sample can hide a
                    # delayed fall during the remaining recovery period.
                    for _ in range(args.off):
                        env.state.info["commands"][:, 4] = 0.0
                        obs = torch.tensor(env.state.obs["obs"], dtype=torch.float32)
                        action = actor(obs).numpy()
                        st = env.step(action)
                        ...
                        z = float(np.asarray(env._backend.get_base_pos())[0, 2])
                        wc = st.info.get("wheel_contact", np.zeros((1, 2)))
                        up = np.asarray(env._backend.get_sensor_data("upvector"), dtype=np.float64)[
                            0
                        ]
                        up_z = float(up[2])
                        if _recovered(wc, z, up_z):
                            ...
                            streak = 0
                    recovered = streak >= args.hold and not bool(st.terminated[0])
        ...
        print(
            f"attempts={n}  airborne(真跳)={n_air}/{n}  recovered(恢复站立)={n_rec}/{n}  "
            f"terminated={n_term}/{n}"
        )
        print(
            f"成功恢复率 = {n_rec / max(n, 1):.2f}  (空中跳恢复率 = "
            f"{sum(1 for r in results if r['airborne'] and r['recovered']) / max(n_air, 1):.2f})"
        )
        if heights:
            print(
                f"跳高(max_z-stand) = {np.mean(heights):.3f} ± {np.std(heights):.3f} m  "
        ...
        print("\n逐跳明细:")
        print(
            f"{'jump':<10}{'air':<5}{'rec':<5}{'term':<5}{'stand':>7}{'maxz':>7}{'height':>8}"
            f"{'air_st':>7}{'spin':>7}{'drift':>7}"
        )
        for r in results:
            h = max(r["max_z"] - r["stand"], 0.0)
            print(
                f"{r['jump']:<10}{str(r['airborne']):<5}{str(r['recovered']):<5}"
