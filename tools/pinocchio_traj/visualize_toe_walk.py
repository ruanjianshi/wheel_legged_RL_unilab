...
def fk_left_leg(angles: np.ndarray, base_pos: np.ndarray = np.zeros(3)):
    ...
    Rx = np.array([[1, 0, 0], [0, np.cos(hip_roll), -np.sin(hip_roll)],
                    [0, np.sin(hip_roll), np.cos(hip_roll)]])
    Ry_thigh = np.array([[np.cos(thigh), 0, np.sin(thigh)], [0, 1, 0],
                          [-np.sin(thigh), 0, np.cos(thigh)]])
    R_thigh = Rx @ Ry_thigh
    knee_pos = hip_pos + R_thigh @ THIGH_OFFSET
    
    # Thigh → calf
    Ry_calf = np.array([[np.cos(calf), 0, np.sin(calf)], [0, 1, 0],
                         [-np.sin(calf), 0, np.cos(calf)]])
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('/home/robot/xiaoq/wheel_legged_RL_unilab/tools/pinocchio_traj/toe_walk_ref.png', dpi=150)
    plt.close()
    ...
    print(f"Saved: tools/pinocchio_traj/toe_walk_ref.png")
