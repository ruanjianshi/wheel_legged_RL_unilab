...
for frame in range(0, num_frames, 5):
    ...
    ax.set_xlim(-0.2, 0.2); ax.set_ylim(-0.3, 0.3); ax.set_zlim(0, 0.8)
    ax.set_title(f'3D (t={t:.2f}s)')
    ax.view_init(elev=20, azim=45)
    
    # Wheel height
    ax2.plot(times, lw_z, 'b-', alpha=0.5); ax2.plot(times, rw_z, 'r--', alpha=0.5)
    ax2.axvline(x=t, color='gray', alpha=0.5)
    ax2.set_ylabel('Z (m)'); ax2.set_title('Wheel Height')
    ...
    ax3.set_xlabel('Time (s)'); ax3.set_ylabel('mm'); ax3.set_title('Clearance')
    ax3.grid(True, alpha=0.3)
    
    # Joint angles
    angles = generate_toe_walk_ref(t)
    ax4.barh(['L hip','L thigh','L calf','L whl','R hip','R thigh','R calf','R whl'], 
             angles, color=['b']*4+['r']*4, alpha=0.7)
    ax4.set_xlim(-0.6, 0.6); ax4.set_title(f'Joint Angles (t={t:.2f}s)')
