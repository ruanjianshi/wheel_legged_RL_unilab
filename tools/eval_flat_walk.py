...
with initialize_config_dir(
    config_dir="/home/robot/xiaoq/wheel_legged_RL_unilab/conf/ppo", version_base="1.3"
):
...
ckpt = torch.load(
    "logs/rsl_rl_ppo/XqRobotV2WalkFlat/2026-06-30_16-09-51_mujoco/model_4999.pt", map_location="cpu"
)
...
actor.load_state_dict(
    {k: v for k, v in ckpt["actor_state_dict"].items() if "distribution" not in k}
)
actor.eval()
...
print(
    f"{'Command':<10} {'Vx':>7} {'Vy':>7} {'tilt°':>7} {'H(m)':>7} | {'L_th':>7} {'R_th':>7} {'L_c':>7} {'R_c':>7}"
)
print("-" * 85)
...
for name, cmd in tests.items():
    ...
    print(
        f"{name:<10} {lv[0]:+7.3f} {lv[1]:+7.3f} {tilt:7.1f} {hz:7.3f} | {dof[1]:+7.2f} {dof[4]:+7.2f} {dof[2]:+7.2f} {dof[5]:+7.2f}"
    )
