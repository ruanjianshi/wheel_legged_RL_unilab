...
Added visual grid overlay and color for height perception.
...
# ── Generate terrain (identical to training config) ──
cfg = XqRobotRoughTerrainCfg()
gen = TerrainGenerator(cfg)
result = gen.generate()
hf = result.heights_yx

print(f"Terrain: {hf.shape[1]}×{hf.shape[0]}, z=[{hf.min():.3f}, {hf.max():.3f}]")
ts = result.terrain_origins
print(f"Cells: {ts.shape[0]}×{ts.shape[1]}")
# List terrain types present
nz = np.count_nonzero(hf)
print(f"Non-flat cells: {nz}/{hf.size}")
...
robot_dir = os.path.expanduser(
    "~/xiaoq/wheel_legged_RL_unilab/src/unilab/assets/robots/xqrobotV2"
)

# Visual: colored hfield + grid lines for height perception
