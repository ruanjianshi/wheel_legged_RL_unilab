...
cfg.reward_config = XqRobotRewardConfig(
    scales={"tracking_lin_vel": 1.5, "tracking_ang_vel": 1.5, "lin_vel_z": -0.2,
            "ang_vel_xy": -0.02, "base_height": -5.0, "orientation": -10.0,
            "joint_action_rate": -0.1, "wheel_action_rate": -0.005,
            "similar_calf": -1.0, "hip_roll": -2.0, "wheel_symmetry": -0.5,
            "tsk": -2.0, "feet_distance": -1.0, "alive": 1.0},
...
# Load model
log_dir = os.path.expanduser(f"~/xiaoq/wheel_legged_RL_unilab/logs/rsl_rl_ppo/XqRobotV2WalkFlat/{RUN}")
if CHECKPOINT:
...
print(f"{'='*78}")
print(f"{'Test':<26} {'CmdVx':>6} {'CmdVy':>6} {'AvgVx':>7} {'AvgVy':>7} {'VxErr':>7} {'VyXtalk':>8}")
print("-" * 78)
...
for name, cmd in tests:
    ...
    print(f"{name:<26} {cmd[0]:>6.2f} {cmd[1]:>6.2f} {avg_vx:>7.3f} {avg_vy:>7.3f} {vx_err:>7.3f} {vy_xtalk:>8.3f}")
