...
from verify_jump import load_actor, _ActorMLP, trained_env_overrides  # reuse loader
...
def main() -> int:
    ...
    if ov is None:
        ...
        ov = {
            "reward_config": {
                "scales": {"jump_height": 12.0, "vertical_thrust": 30.0,
                           "crouch_prep": 4.0, "landing_soft": 15.0,
                           "wheel_air_time": 20.0, "crouch_depth": 4.0,
                           "alive": 1.0, "base_height": -60.0,
                           "orientation": -5.0, "joint_action_rate": -0.1,
                           "wheel_action_rate": -0.015, "leg_mirror": -12.0,
                           "tsk": -2.0, "anti_loiter": 12.0, "lean_forward": 5.0,
                           "tracking_lin_vel": 2.0, "tracking_ang_vel": 1.0,
                           "lin_vel_z": -0.2, "ang_vel_xy": -0.05},
                "tracking_sigma": 0.3, "base_height_target": 0.55,
                "jump_height_target": 1.0, "crouch_height_target": 0.40,
                "max_tilt_deg": 60.0, "min_base_height": 0.15,
                "jump_curriculum_start": 0, "jump_curriculum_end": 0,
            },
            "domain_rand": {"randomize_base_mass": False,
                            "randomize_ground_friction": False,
                            "randomize_kp": False, "randomize_kd": False,
                            "random_com": False, "randomize_leg_length": False},
            "control_config": ctrl,
        }
    env = registry.make(
        args.task, num_envs=1, sim_backend="mujoco",
        env_cfg_override=ov,
    )
    ...
    print(f"step  trig  base_z  wl_contact wr_contact  phase  fsm   maxz")
    maxz = 0.0
    with torch.no_grad():
        for step in range(args.steps):
            ...
            fsm = float(getattr(env, "_fsm_state", np.zeros(1))[0] if hasattr(env, "_fsm_state") else 0)
            if step % 10 == 0 or base_z > 0.75:
                print(f"{step:5d}  {int(trigger)}   {base_z:.3f}  "
