...
用法:
  uv run python scripts/xqrobotwl/render_trained_unicycle.py \
...
def main() -> None:
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-framerate", "50", "-i", str(tmp / "f_%05d.ppm"),
         "-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "fast", str(out_path)],
