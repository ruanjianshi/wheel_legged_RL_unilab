...
用法:
  uv run mjpython scripts/xqrobotwl/diag_runtime_rollout.py \
...
def main() -> None:
    ...
    wrapper_cls = resolve_rsl_rl_ppo_runtime(rl_cfg, default_wrapper_cls=RslRlVecEnvWrapper).wrapper_cls
    ...
    env = registry.make(
        "XqRobotWLFallRecoveryFlat", sim_backend="mujoco", num_envs=args.num_envs, env_cfg_override=env_cfg_override
    )
    ...
    for i in range(args.steps):
        ...
        rec |= bz > 0.45
        obs = st.obs
    print(f"runtime rollout: max_z={maxz.mean():.2f}  达0.45: {rec.mean()*100:.0f}%  达0.55: {(maxz>0.55).mean()*100:.0f}%")
