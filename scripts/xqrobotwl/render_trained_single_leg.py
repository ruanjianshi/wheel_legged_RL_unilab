"""渲染训练后的单腿平衡策略回放 — 跟踪相机 mp4

用法:
  uv run python scripts/xqrobotwl/render_trained_single_leg.py \
      --ckpt logs/rsl_rl_ppo/XqRobotWLSingleLegFlat/<run>/model_6000.pt \
      --out video/single_leg/2026-08-06_run12_6000轮_训练策略.mp4 \
      --steps 600
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="渲染训练后单腿平衡策略")
    ...
    ap.add_argument("--num_envs", type=int, default=4, help="渲染 env 数")
    args = ap.parse_args()
    ...
    # start_in_balance 由 config 控制: reset 直接置单轮平衡位 (FSM 状态1)
    ...
    # 渲染 (跟踪相机: 跟随机器人, 侧面视角看 30° 侧压 + 前后震荡)
    ...
    out_path = Path(args.out) if args.out else ROOT / "video" / "single_leg" / "trained.mp4"
