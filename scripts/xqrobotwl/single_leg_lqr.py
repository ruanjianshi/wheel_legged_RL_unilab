...
用法:
  uv run mjpython scripts/xqrobotwl/single_leg_lqr.py               # 测动力学 + 闭环仿真
  uv run mjpython scripts/xqrobotwl/single_leg_lqr.py --render ...  # 渲染 mp4
"""
...
def main() -> None:
    ...
    if args.search:
        ...
        best = None
        for k1, k2, kr1 in itertools.product([110, 130, 150, 180], [12, 15, 18, 22], [0.5, 1.0, 1.5]):
    ...
    hold, _ = run_closed_loop(model, dt, args.k1, args.k2, args.kr1, args.kr2, args.sim_time, render_dir, k3=args.k3)
    print(f"  K_p=[{args.k1},{args.k2}] K_r=[{args.kr1},{args.kr2}] → 保持 {hold:.2f}s / {args.sim_time}s")
    print(f"  {'✅ ≥2s, LQR 参考可行!' if hold >= 2.0 else '⚠️ 未达 2s, 需调增益'}")
    if render_dir is not None and len(list(render_dir.glob("f_*.ppm"))) > 0:
        out = Path(args.render)
        subprocess.run(
            ["ffmpeg", "-y", "-framerate", "50", "-i", str(render_dir / "f_%05d.ppm"),
