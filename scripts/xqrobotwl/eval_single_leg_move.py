...
用法:
  uv run mjpython scripts/xqrobotwl/eval_single_leg_move.py \
...
def main() -> None:
    ...
    active = np.arange(args.num_envs)
    print("=" * 60)
    print(f"评估: {args.ckpt}")
    print("=" * 60)
    print(f"  episode 长度: {ep_len.mean():.1f} 步 / {args.max_steps}  ({(ep_len>=args.max_steps).mean()*100:.0f}% 跑满)")
