...
class EvalContext:
    ...
    def record(
        self, linvel, gyro, base_z, leg_pos, leg_vel, action, torque, wheel_vel, roll, pitch
    ):
...
def vel_tracking_ratio(ctx: EvalContext) -> float:
    ...
    if cmd_norm < 1e-6:
        return float(
            np.mean([np.sqrt(vx**2 + vy**2) for vx, vy in zip(ctx.actual_vx, ctx.actual_vy)])
        )
...
def action_smoothness(ctx: EvalContext) -> float:
    ...
    if len(ctx.actions) < 2:
        return float("nan")
    diffs = [
        np.linalg.norm(ctx.actions[i] - ctx.actions[i - 1]) for i in range(1, len(ctx.actions))
    ]
...
def cost_of_transport(ctx: EvalContext) -> float:
    ...
    mass = 5.0
    g = 9.81
    speed = np.mean([np.sqrt(vx**2 + vy**2) for vx, vy in zip(ctx.actual_vx, ctx.actual_vy)])
    if speed < 0.01:
        return 0.0
