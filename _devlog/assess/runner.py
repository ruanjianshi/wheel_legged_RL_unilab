...
def build_env_stairs():
    ...
    cfg.control_config.wheel_action_scale = 8.0
    cfg.control_config.clip_actions = 100.0
    cfg.commands = Commands(
        vel_limit=[[-1.0, 0.0, -1.5, -0.1, 0.40], [1.0, 0.0, 1.5, 0.1, 0.90]],
        resampling_time=999.0,
    )
    cfg.reward_config = XqRobotRewardConfig(
        scales={
    ...
    cfg.curriculum.enabled = False
    cfg.terrain_curriculum.enabled = False
    return XqRobotV2StairsEnv(cfg, num_envs=1, backend_type="mujoco")


register_env_builder("stairs", build_env_stairs)
...
def build_env_jump():
    ...
    cfg.control_config.clip_actions = 100.0
    cfg.commands = Commands(
        vel_limit=[[-0.3, 0.0, -0.5, -0.1, 0], [0.3, 0.0, 0.5, 0.1, 1]],
        resampling_time=999.0,
    )
    cfg.reward_config = XqRobotJumpRewardConfig(
        scales={
    ...
    cfg.domain_rand.random_com = False
    cfg.curriculum.enabled = False
    return XqRobotV2JumpFlatEnv(cfg, num_envs=1, backend_type="mujoco")


register_env_builder("jump", build_env_jump)


def load_policy(ckpt_path: str, obs_dim: int, hidden_dims: list[int] | None = None) -> MLP:
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=True)
    mlp_state = {k[4:]: v for k, v in ckpt["actor_state_dict"].items() if k.startswith("mlp.")}
    if hidden_dims is None:
        dims = _infer_dims_from_state(mlp_state, obs_dim, 8)
    else:
        dims = hidden_dims
    mlp = MLP(obs_dim, 8, dims, activation="elu")
