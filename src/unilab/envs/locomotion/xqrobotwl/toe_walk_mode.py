...
_LIFT_ONLY_REWARD_KEYS = (
    "phase_swing_lift",  # v8: 加回 mode 门控 (v5 移除后 v6/v7 加回奖励但漏了掩码 → 站立模式也拿抬腿奖)
    "phase_knee_lift",
    "phase_knee_stance",
...
def _reward_wheel_still(ctx: RewardContext) -> np.ndarray:
    """零指令时罚轮子转动 (逼静态站立; 两模式无命令都必须静止)."""
    assert ctx.dof_vel is not None
    speed = np.sum(np.abs(ctx.dof_vel[:, -2:]), axis=1)
    commands = np.asarray(ctx.info.get("commands", np.zeros((ctx.num_envs, 5))))
    stopped = np.linalg.norm(commands[:, :3], axis=1) < 0.05
    return -np.clip(speed - 1.0, 0.0, None) * stopped.astype(get_global_dtype())


def _reward_stand_contact(ctx: RewardContext) -> np.ndarray:
    """站立模式 (mode=0) 罚轮子离地 — 站立必须双轮着地."""
    mode = _mode_column(ctx)
    wc = np.asarray(ctx.info.get("wheel_contact", np.zeros((ctx.num_envs, 2))))
    off = np.sum(1.0 - wc, axis=1)
    return -off * (1.0 - mode)


def _tracking_gated_by_command(fn):
    """追踪奖励包装: 仅当有速度/转向指令时才给分 (零指令不再白拿 4/步)."""

    def wrapped(ctx: RewardContext) -> np.ndarray:
        out = np.asarray(fn(ctx), dtype=get_global_dtype())
        commands = np.asarray(ctx.info.get("commands", np.zeros((ctx.num_envs, 5))))
        active = (
            (np.abs(commands[:, 0]) > 0.05)
            | (np.abs(commands[:, 1]) > 0.05)
            | (np.abs(commands[:, 2]) > 0.05)
        )
        return out * active.astype(get_global_dtype())
...
class XqRobotWLToeWalkModeEnv(XqRobotWLToeWalkFlatEnv):
    ...
    def _sample_mode_values(self, num_samples: int) -> np.ndarray:
        ...
        # v9 Stage3: 抬腿 70% / 站立 30% (抬腿是难任务需更多样本, 站立 30% 足够维持)
        return (np.random.rand(num_samples) < 0.7).astype(dtype)
    ...
    def _init_reward_functions(self) -> None:
        ...
        # v7: 站立静态约束 + 零指令追踪门控
        self._reward_fns["wheel_still"] = _reward_wheel_still  # 零指令轮静止 (两模式)
        self._reward_fns["stand_contact"] = _reward_stand_contact  # mode=0 轮必须着地
        for key in ("tracking_lin_vel", "tracking_ang_vel"):
            if key in self._reward_fns:
                self._reward_fns[key] = _tracking_gated_by_command(self._reward_fns[key])
