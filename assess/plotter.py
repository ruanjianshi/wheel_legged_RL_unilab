...
def plot_metric_bars(
    ...
    for i, metric in enumerate(metric_names):
        ax = axes[i // cols] if cols == 1 else axes[i // cols][i % cols] if rows > 1 else axes[i % cols]
        values = []
        labels = []
...
def plot_stability_timeline(
    ...
    for idx, (name, rec) in enumerate(records.items()):
        ...
        # Orientation
        axes[idx, 1].plot(t, np.rad2deg(rec.base_roll), color=COLORS["red"], label="Roll", alpha=0.7)
        axes[idx, 1].plot(t, np.rad2deg(rec.base_pitch), color=COLORS["blue"], label="Pitch", alpha=0.7)
...
def plot_metric_comparison(
    ...
    for i, model in enumerate(models):
        ...
        ax.bar(x + i * bar_w, vals, bar_w,
               color=colors_list[i % len(colors_list)], alpha=0.8, label=model[:20])
...
def plot_gait_phase(
    ...
    for i, ax in enumerate(axes.flat):
        if i < pos_arr.shape[1]:
            ...
            ax.scatter(pos_arr[0, i], vel_arr[0, i], color=COLORS["green"], s=30, zorder=5, label="start")
            ax.scatter(pos_arr[-1, i], vel_arr[-1, i], color=COLORS["red"], s=30, zorder=5, label="end")
